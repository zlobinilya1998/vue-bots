<template>
  <header class="header" :style="{borderBottom: headerBorder}">
    <Logo />
    ...
    >
      <rect width="30" height="3.44828" rx="1.72414" fill="#C4C4C4" />
      <rect
        y="8.27586"
        width="30"
        height="3.44828"
    ...
    <transition name="menu">
      <div v-if="showMobileMenu" class="mobile-menu">
        <div @click="setShowBasket(true)" class="cart-icon mobile">
          <svg
            width="20"
            height="20"
            viewBox="0 0 20 20"
        ...
        >
          <path
            :fill="$route.path === '/favorite' ? '#9DD458' : '#9B9B9B'"
        ...
        >
          <path
            :fill="$route.path === '/profile' ? '#9DD458' : '#9B9B9B'"
            fill-rule="evenodd"
            clip-rule="evenodd"
...
export default {
  ...
  computed: {
    headerBorder() {
      return this.menuShow ? "2px solid #D1F6FF" : "2px solid #9dd458";
    },
    menuShow(){
      return this.$route.matched.find(item=>item.name === 'profile');
    }
  },
  methods: {
    showMenu() {
...
.menu-enter, .menu-leave-to /* .fade-leave-active до версии 2.1.8 */ {
  opacity: 0;
}
</style>
